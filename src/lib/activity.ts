...
// A portal reply arrives as a thread comment headed "<…> from <email>:". The email is the real author;
// the header line is plumbing. No header → the generic portal author.
export function portalComment(text: string): { author: string; body: string } {
	const m = /^[^\n]*?\bfrom\s+([^\s@:]+@[^\s:]+):[ \t]*\r?\n?/i.exec(text);
	return m ? { author: m[1], body: text.slice(m[0].length).trim() } : { author: 'Client (portal)', body: text.trim() };
}
