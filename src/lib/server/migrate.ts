...
export const MIGRATIONS: ((db: Database) => void)[] = [
	...
	// 5: meeting recordings become notes (server/recordings.ts), filed by who attended. Companies keep their
	// website domain so an attendee's email can find the client; clearing the sync stamp makes the next
	// company sync a full pull, which fills the domain in for companies that haven't changed since.
	(db) => {
		addCol(db, 'companies', 'domain', 'TEXT');
		db.exec(`CREATE INDEX IF NOT EXISTS companies_domain_idx ON companies(domain);
			CREATE TABLE IF NOT EXISTS recordings (item_id TEXT PRIMARY KEY, folder TEXT NOT NULL, file TEXT NOT NULL, created_at TEXT NOT NULL);
			DELETE FROM meta WHERE key = 'companies_synced_at'`);
	},
	// 6: a recording note also gets the meeting's transcript, fetched once Teams has it. `transcript` is
	// 'pending' until then, 'done', or 'none' (no transcript, or none within the retry window).
	(db) => {
		addCol(db, 'recordings', 'recorded_at', 'TEXT');
		addCol(db, 'recordings', 'join_url', 'TEXT');
		addCol(db, 'recordings', 'transcript', "TEXT NOT NULL DEFAULT 'pending'");
	}
];
