...
// ---- @mentions. The composer sends plain text with "@Name" in it plus the people it picked; each
// "@Name" becomes a Teams <at id="n"> tag with a matching entry in the message's `mentions` array.
export type Mention = { id: string; name: string };
/** Well-formed mentions from a request body: an Entra object id and a display name. */
export function pickMentions(v: unknown): Mention[] {
	if (!Array.isArray(v)) return [];
	return v
		.flatMap((m: unknown) => {
			const o = m && typeof m === 'object' ? (m as Record<string, unknown>) : {};
			return typeof o.id === 'string' && /^[0-9a-f-]{36}$/i.test(o.id) && typeof o.name === 'string' && o.name.trim() ? [{ id: o.id, name: o.name.trim() }] : [];
		})
		.slice(0, 20);
}
/** `@Name` in already-escaped HTML → `<at>` tags, plus the `mentions` array Graph needs beside them. */
export function applyMentions(html: string, people: Mention[]) {
	const byName = new Map(people.map((p) => [escape(p.name), p]));
	const mentions: { id: number; mentionText: string; mentioned: { user: { id: string; displayName: string; userIdentityType: 'aadUser' } } }[] = [];
	if (!byName.size) return { html, mentions };
	// longest first, so "@Ann Lee" isn't taken as "@Ann"; a name must end at a non-letter
	const names = [...byName.keys()].sort((a, b) => b.length - a.length).map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
	const out = html.replace(new RegExp(`@(${names.join('|')})(?![\\p{L}\\p{N}])`, 'gu'), (_, name: string) => {
		const p = byName.get(name)!;
		const id = mentions.length;
		mentions.push({ id, mentionText: p.name, mentioned: { user: { id: p.id, displayName: p.name, userIdentityType: 'aadUser' } } });
