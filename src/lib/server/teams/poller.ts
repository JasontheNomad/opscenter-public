...
import { syncRecordings } from '../recordings';
...
export function startTeamsPoller() {
	...
	const warmPhotos = async (chats: ChatSummary[], myId: string) => {
		// the recent rows only: older pages sit behind "Show more", and warming ~200 chats' photos cost more than it saved
		const ids = [...new Set(chats.slice(0, 40).flatMap((c) => c.people.filter((p) => p.id !== myId).slice(0, 2).map((p) => p.id)))];
	...
	const loop = async () => {
		...
		try {
			...
			if (every(5 * MIN)) await syncRecordings();
		} catch (e) {
