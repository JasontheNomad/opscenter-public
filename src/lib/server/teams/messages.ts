...
import { applyMentions, escape, fileProxy, isImage, proxied, rebuildBody, toMsg, type ChatMessage, type Mention, type Quote, type RawMsg } from './render';
...
// Graph pages chats ~25 at a time whatever $top says. New messages move a chat onto page 1, so every
// tick reads page 1; older pages (the sidebar's "Show more") refresh every 5 min.
type ChatPage = { value: RawChat[]; '@odata.nextLink'?: string };
const OLDER_MS = 5 * MIN;
const MAX_PAGES = 8;
let older = { at: 0, v: [] as RawChat[] };
export async function listChats(): Promise<ChatSummary[]> {
	if (DEMO()) return demoChats.map((c) => ({ ...c }));
	const first = await graph<ChatPage>('/me/chats?$expand=lastMessagePreview,members&$top=50&$orderby=lastMessagePreview/createdDateTime desc');
	if (Date.now() - older.at > OLDER_MS) {
		const rows: RawChat[] = [];
		let url = first['@odata.nextLink'] ?? null;
		for (let page = 1; url && page < MAX_PAGES; page++) {
			const r: ChatPage = await graph(url);
			rows.push(...r.value);
			url = r['@odata.nextLink'] ?? null;
		}
		older = { at: Date.now(), v: rows };
	}
	// page 1 wins over an older copy of the same chat
	const onFirst = new Set(first.value.map((c) => c.id));
	return [...first.value, ...older.v.filter((c) => !onFirst.has(c.id))]
...
async function messagePayload(text: string, quote?: Quote | null, uploads: Upload[] = [], people: Mention[] = []) {
	const tagged = applyMentions(escape(text), people);
	let html = linkUrls(tagged.html).replace(/\n/g, '<br>');
...
export const sendChatMessage = async (chatId: string, text: string, quote?: Quote | null, uploads: Upload[] = [], people: Mention[] = []) =>
	graph<{ id: string }>(`/chats/${encodeURIComponent(chatId)}/messages`, { method: 'POST', body: JSON.stringify(await messagePayload(text, quote, uploads, people)) });
...
export const markChatRead = (chatId: string, userId: string) =>
	...
	}).catch((e) => console.warn('[teams] markChatRead failed:', errMsg(e).slice(0, 160)));
...
export const sendThreadReply = async (teamId: string, channelId: string, messageId: string, text: string, uploads: Upload[] = [], people: Mention[] = []) =>
	channelChanged(channelId, graph<{ id: string }>(`/teams/${encodeURIComponent(teamId)}/channels/${encodeURIComponent(channelId)}/messages/${encodeURIComponent(messageId)}/replies`, { method: 'POST', body: JSON.stringify(await messagePayload(text, null, uploads, people)) }));

// channel: quoting a post = replying in its thread
export const sendChannelMessage = async (teamId: string, channelId: string, text: string, quote?: Quote | null, uploads: Upload[] = [], people: Mention[] = []) =>
	channelChanged(channelId, quote
		? graph<{ id: string }>(`/teams/${encodeURIComponent(teamId)}/channels/${encodeURIComponent(channelId)}/messages/${encodeURIComponent(quote.id)}/replies`, { method: 'POST', body: JSON.stringify(await messagePayload(text, null, uploads, people)) })
		: graph<{ id: string }>(`/teams/${encodeURIComponent(teamId)}/channels/${encodeURIComponent(channelId)}/messages`, { method: 'POST', body: JSON.stringify(await messagePayload(text, null, uploads, people)) }));
