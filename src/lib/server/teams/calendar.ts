...
// ---- auto-record: every upcoming Teams meeting Jason organizes gets "Record and transcribe automatically".
// Without Teams Premium there's no per-user default — the option is per meeting and starts off — so the
// app sets it, HubSpot bookings included. Only the organizer can change it, and it doesn't apply to a
// meeting already in progress. Each join link is handled once per process; a failure retries later.
const RECORD_RETRY_MS = 15 * 60_000;
const recordChecked = new Map<string, number>(); // joinUrl -> next time to look (Infinity = done)
async function autoRecord() {
	const now = Date.now();
	const events = (await Promise.all([0, 1].map((o) => calendarWeek(weekOf(null, o)).catch(() => [] as CalEvent[])))).flat();
	for (const e of events) {
		if (!e.isOrganizer || !e.joinUrl || e.cancelled || Date.parse(e.start) <= now) continue;
		if ((recordChecked.get(e.joinUrl) ?? 0) > now) continue;
		try {
			const q = new URLSearchParams({ $filter: `JoinWebUrl eq '${e.joinUrl.replace(/'/g, "''")}'` });
			const m = (await graph<{ value: { id: string; recordAutomatically?: boolean }[] }>(`/me/onlineMeetings?${q}`)).value[0];
			if (m && !m.recordAutomatically)
				await graph<unknown>(`/me/onlineMeetings/${m.id}`, { method: 'PATCH', body: JSON.stringify({ recordAutomatically: true }) });
			recordChecked.set(e.joinUrl, Infinity);
		} catch (err) {
			recordChecked.set(e.joinUrl, now + RECORD_RETRY_MS);
			console.warn('[calendar] auto-record skipped:', errMsg(err).slice(0, 200));
		}
	}
}
onLogout(() => recordChecked.clear());
...
// someone on both lists is required
const optionalOnly = (required: string[], optional: string[] = []) => {
	const req = new Set(required.map((a) => a.toLowerCase()));
	return optional.filter((a) => !req.has(a.toLowerCase()));
};
...
// `allDay`: whole days in one zone, `end` = the last day included. Graph wants midnight to midnight, end exclusive.
export const createMeeting = (o: { subject: string; startISO: string; endISO: string; attendees: string[]; optional?: string[]; location?: string; body?: string; online?: boolean; allDay?: { start: string; end: string; timeZone: string }; showAs?: 'busy' | 'oof' }) =>
	changed(graph<{ id: string; onlineMeeting?: { joinUrl?: string } }>('/me/events', {
		...
		body: JSON.stringify({
			...
			...(o.allDay
				? { isAllDay: true, start: { dateTime: `${o.allDay.start}T00:00:00`, timeZone: o.allDay.timeZone }, end: { dateTime: `${nextDay(o.allDay.end)}T00:00:00`, timeZone: o.allDay.timeZone } }
				: { start: { dateTime: o.startISO, timeZone: 'UTC' }, end: { dateTime: o.endISO, timeZone: 'UTC' } }),
			showAs: o.showAs ?? 'busy',
			attendees: [
				...o.attendees.map((a) => ({ emailAddress: { address: a }, type: 'required' })),
				...optionalOnly(o.attendees, o.optional).map((a) => ({ emailAddress: { address: a }, type: 'optional' }))
			],
...
export async function getMeeting(id: string) {
	...
	const person = (a: NonNullable<RawMeeting['attendees']>[number]) =>
	...
	return {
		...
		// required (and rooms) vs optional, the two invite rows in the editor
		attendees: (e.attendees ?? []).filter((a) => a.type !== 'optional').flatMap(person),
		optional: (e.attendees ?? []).filter((a) => a.type === 'optional').flatMap(person)
	};
}

export async function updateMeeting(id: string, o: { subject: string; attendees: string[]; optional?: string[]; location: string; body?: string; start?: { dateTime: string; timeZone: string }; end?: { dateTime: string; timeZone: string } }) {
	...
	// with an optional list the two rows decide (a room stays a room); without one — an editor open from
	// before the optional row existed — each existing attendee keeps its type and new ones are required
	const requiredType = (a: string) => {
		const t = typeOf.get(a.toLowerCase());
		return o.optional ? (t === 'resource' ? t : 'required') : (t ?? 'required');
	};
	const patch: Record<string, unknown> = {
		subject: o.subject,
		location: { displayName: o.location },
		attendees: [
			...o.attendees.map((a) => ({ emailAddress: { address: a }, type: requiredType(a) })),
			...optionalOnly(o.attendees, o.optional).map((a) => ({ emailAddress: { address: a }, type: 'optional' }))
		]
	};
