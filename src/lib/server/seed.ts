...
// A thread can exist that was never saved here: seeding skips tickets that predate it, and a reply from
// HubSpot's own composer opens one. Look it up and keep it — without it a reply reaches nobody and the
// client's portal replies (thread comments) can't be read.
export async function savedThreadId(task: { id: number; hs_id: string; hs_thread_id: string | null }): Promise<string | null> {
	if (task.hs_thread_id) return task.hs_thread_id;
	const threadId = await ticketThreadId(task.hs_id);
	if (threadId) setThread.run(threadId, task.id);
	return threadId;
}
