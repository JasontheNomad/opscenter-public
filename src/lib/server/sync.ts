...
import { OWNER_ID, getPipelines, searchTickets, fetchCompanies, fetchContacts, latestActivity, recordUrl, type Pipeline } from './hubspot';
import type { Status } from '$lib/columns';
import { PROJECT_PIPELINE_LABEL, columnsForPipeline } from '$lib/views';
import type { Source } from '$lib/types';
import { errMsg } from '$lib/api';
import { ensureClientFolders } from './notes';
...
async function syncTickets() {
	...
	// a ticket not mirrored before this run gives its company a client folder, if it has none
	const known = new Set(db.prepare("SELECT hs_id FROM tasks WHERE source = 'ticket'").pluck().all());
	upsertMirrors('ticket', rows);
	ensureClientFolders(rows.filter((r) => r.company_id && !known.has(r.hs_id)).map((r) => r.company_id!));
...
async function syncLatest(ticketIds: string[]) {
	try {
		...
		const threads = (
			db.prepare("SELECT hs_id, hs_thread_id FROM tasks WHERE source = 'ticket' AND hs_thread_id IS NOT NULL AND status <> 'done'").all() as { hs_id: string; hs_thread_id: string }[]
		)
			.filter((r) => ids.has(r.hs_id))
			.map((r): [string, string] => [r.hs_id, r.hs_thread_id]);
		const latest = await latestActivity(ticketIds, threads);
		...
		console.warn('[sync] latest activity skipped:', errMsg(e).slice(0, 160));
	}
}

const upsertCompanies = db.transaction((rows: { id: string; name: string; domain: string | null }[]) => {
	// upsert, not REPLACE: REPLACE deletes the row first, which wiped the local `notes_dir` folder link
	const up = db.prepare('INSERT INTO companies (id, name, domain) VALUES (?, ?, ?) ON CONFLICT(id) DO UPDATE SET name = excluded.name, domain = excluded.domain');
	for (const c of rows) up.run(c.id, c.name, c.domain);
	return rows.length;
});
...
async function syncRef<Row>(
	...
	try {
		...
		console.warn(`[sync] ${name} skipped:`, errMsg(e).slice(0, 160));
		return 'skipped';
	}
}

const syncCompanies = () => syncRef('companies', fetchCompanies, upsertCompanies);
...
const syncContacts = () => syncRef('contacts', fetchContacts, upsertContacts);
