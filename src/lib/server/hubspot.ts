...
import { portalComment } from '$lib/activity';
import { normalizeDomain } from '$lib/recordings';
import { errMsg } from '$lib/api';
import { chunk } from './teams/shared';
...
export async function fetchContacts(since?: string) {
...
// `domain` (the website, normalized) lets a meeting attendee's email find the client.
export async function fetchCompanies(since?: string): Promise<{ id: string; name: string; domain: string | null }[]> {
	const props = ['name', 'domain'];
	const recs = since
		? await searchAll('companies', [{ propertyName: 'hs_lastmodifieddate', operator: 'GT', value: String(Date.parse(since)) }], props)
		: await listAll('companies', props);
	return recs.map((r) => ({ id: r.id, name: r.properties.name ?? `Company ${r.id}`, domain: normalizeDomain(r.properties.domain) }));
}
...
type ThreadMessage = { id: string; type: string; createdAt: string; text?: string; client?: { clientType?: string }; attachments?: { fileId?: string }[] };

// The client portal posts a client's reply into the ticket's conversation thread as a COMMENT from its
// integration — not a note, not an email — so it only exists in the thread. Newest 100 messages.
async function portalComments(threadId: string): Promise<ThreadMessage[]> {
	try {
		const r = await hs<{ results: ThreadMessage[] }>(`/conversations/v3/conversations/threads/${threadId}/messages?limit=100`);
		return r.results.filter((m) => m.type === 'COMMENT' && m.client?.clientType === 'INTEGRATION');
	} catch (e) {
		console.warn('[hubspot] thread comments skipped:', errMsg(e).slice(0, 160));
		return []; // a deleted thread must not break the whole panel
	}
}

export async function ticketActivity(ticketId: string, threadId?: string | null): Promise<Activity[]> {
	...
	const toComment = async (m: ThreadMessage): Promise<Activity> => ({
		id: m.id,
		kind: 'note',
		at: m.createdAt,
		...portalComment(m.text ?? ''),
		portal: true,
		attachments: await attachmentsOf((m.attachments ?? []).map((a) => a.fileId ?? '').join(';'))
	});
	const comments = threadId ? await portalComments(threadId) : [];
	const out = await Promise.all([...noteRows.map(toNote), ...emailRows.map(toEmail), ...comments.map(toComment)]);
...
// threads: [ticketId, threadId] for tickets with a saved thread — portal replies live only there
export async function latestActivity(ticketIds: string[], threads: [string, string][] = []): Promise<Map<string, Latest>> {
	...
	await Promise.all(
		threads.map(async ([t, threadId]) => {
			for (const m of await portalComments(threadId)) bump(t, 'last_client_at', m.createdAt);
		})
	);
	return out;
}
