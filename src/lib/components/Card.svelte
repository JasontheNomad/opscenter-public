<script lang="ts">
	...
	import { parseChecklist, isUnseen, needsReply, CHANGE_LABEL, type Task } from '$lib/types';
...
>
	...
	{#if task.source !== 'manual' || task.due_date || task.company_name || list.length}
		<div class="mt-1.5 flex items-center gap-2 pl-3.5 text-caption text-muted">
			...
			{#if isUnseen(task)}<span class="rounded bg-alert/20 px-1 py-px font-medium text-alert-soft">{(CHANGE_LABEL[task.hs_change ?? ''] ?? 'Changed').toLowerCase()}</span>{/if}
