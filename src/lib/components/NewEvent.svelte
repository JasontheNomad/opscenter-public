<script lang="ts">
	...
	// new events only: all day spans `date`..`endDate` (both included); showAs is Outlook's Busy / Out of office
	let allDay = $state(false);
	let endDate = $state(localInput(s0).slice(0, 10));
	let showAs = $state<'busy' | 'oof'>('busy');
	let body = $state('');
	...
	const valid = $derived(subject.trim().length > 0 && (allDay ? endDate >= date : end > start) && !loading);
	let orig = { date: '', startT: '', endT: '', body: '' };
	$effect(() => {
		if (!edit) return;
		const { id } = edit;
		api<{ subject: string; start: string; end: string; location: string; body: string; online: boolean; attendees: Person[]; optional?: Person[] }>(`/api/calendar/event/${encodeURIComponent(id)}`)
			.then((m) => {
				const s = localInput(new Date(m.start)), e = localInput(new Date(m.end));
				subject = m.subject; location = m.location; body = m.body; online = m.online;
				required.list = m.attendees; optional.list = m.optional ?? [];
	...
	// attendee rows (required, optional): chips + directory search each. A person is on one row only.
	type Person = { name: string; mail: string };
	type Hit = Person & { id: string };
	type Invitees = { list: Person[]; q: string; hits: Hit[]; active: number; placeholder: string };
	const invitees = (placeholder: string): Invitees => {
		const f = $state<Invitees>({ list: [], q: '', hits: [], active: 0, placeholder });
		return f;
	};
	const required = invitees('Invite required attendees');
	const optional = invitees('Invite optional attendees');
	const invited = (mail: string) => [...required.list, ...optional.list].some((a) => a.mail === mail);
	let timer: ReturnType<typeof setTimeout>;
	function onQ(f: Invitees, v: string) {
		f.q = v;
		clearTimeout(timer);
		if (v.trim().length < 2) { f.hits = []; return; }
		timer = setTimeout(async () => {
			const people = await api<Hit[]>(`/api/calendar/people?q=${encodeURIComponent(v.trim())}`).catch(() => []);
			f.hits = people.filter((h) => !invited(h.mail));
			f.active = 0;
		}, 200);
	}
	function pick(f: Invitees, h: Person) {
		if (!invited(h.mail)) f.list = [...f.list, { name: h.name, mail: h.mail }];
		f.q = ''; f.hits = [];
	}
	function qKeys(f: Invitees, e: KeyboardEvent) {
		if (e.key === 'ArrowDown') (e.preventDefault(), (f.active = Math.min(f.active + 1, f.hits.length - 1)));
		else if (e.key === 'ArrowUp') (e.preventDefault(), (f.active = Math.max(f.active - 1, 0)));
		else if (e.key === 'Enter') {
			e.preventDefault();
			if (f.hits[f.active]) pick(f, f.hits[f.active]);
			else if (isEmail(f.q.trim())) pick(f, { name: f.q.trim(), mail: f.q.trim() });
		} else if (e.key === 'Backspace' && !f.q && f.list.length) f.list = f.list.slice(0, -1);
	}
	const mails = (f: Invitees) => f.list.map((a) => a.mail);
		...
		try {
			if (edit) {
				...
				await api(`/api/calendar/event/${encodeURIComponent(edit.id)}`, 'PATCH', {
					subject: subject.trim(), attendees: mails(required), optional: mails(optional), location,
					...(body !== orig.body ? { body } : {}),
					...(moved ? { start: { dateTime: `${date}T${startT}:00`, timeZone }, end: { dateTime: `${date}T${endT}:00`, timeZone } } : {})
				});
			} else {
				const when = allDay
					? { allDay: true, start: date, end: endDate, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone }
					: { start: start.toISOString(), end: end.toISOString() };
				await post('/api/calendar/new', { subject: subject.trim(), ...when, showAs, attendees: mails(required), optional: mails(optional), location, body, online });
			}
			oncreated();
		} catch (e) {
			err = errMsg(e);
...
{#snippet inviteRow(f: Invitees, icon: string)}
	<div class="relative flex items-start gap-3 border-b border-border py-2.5">
		<span class="mt-1 w-6 text-center text-muted">{icon}</span>
		<div class="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
			{#each f.list as a (a.mail)}
				<span class="flex items-center gap-1 rounded-full bg-surface-2 py-0.5 pr-1 pl-2.5 text-xs text-text" title={a.mail}>{a.name}<button class="rounded-full px-1 link-muted" onclick={() => (f.list = f.list.filter((x) => x.mail !== a.mail))}>×</button></span>
			{/each}
			<input value={f.q} oninput={(e) => onQ(f, e.currentTarget.value)} onkeydown={(e) => qKeys(f, e)} placeholder={f.list.length ? '' : f.placeholder} class="min-w-40 flex-1 bg-transparent py-1 text-md text-text placeholder:text-muted focus:outline-none" />
		</div>
		{#if f.hits.length}
			<div class="absolute top-full left-9 z-10 mt-1 w-80 rounded-md border border-border bg-surface-2 py-1 shadow-lg">
				{#each f.hits as h, i (h.id)}
					<button class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs {i === f.active ? 'bg-surface' : ''} hover:bg-surface" onmousedown={(e) => (e.preventDefault(), pick(f, h))}>
	...
	<div class="flex min-h-0 flex-1">
		...
		<div class="flex min-w-0 flex-1 flex-col gap-1 overflow-y-auto p-5">
			<div class="rounded-lg bg-surface p-4">
				...
				{@render inviteRow(required, '👥')}
				{@render inviteRow(optional, '')}

				<div class="flex flex-wrap items-center gap-3 border-b border-border py-2.5">
					<span class="w-6 text-center text-muted">🕒</span>
					...
					{#if allDay}
						...
						<input type="date" bind:value={endDate} min={date} class="rounded-md bg-surface-2 px-2 py-1 text-md text-text focus:outline-none" />
						{#if endDate < date}<span class="err">end must be on or after start</span>{/if}
					{:else}
					...
					{/if}
					<!-- all day + show as: new events only (an existing event keeps its kind; edit busy/OOO in Outlook) -->
					{#if !edit}
						<button type="button" role="switch" aria-checked={allDay} aria-label="All day" class="relative h-6 w-11 rounded-full transition-colors {allDay ? 'bg-accent' : 'bg-border-2'}" onclick={() => { allDay = !allDay; if (endDate < date) endDate = date; }}>
							<span class="absolute top-0.5 size-5 rounded-full bg-white transition-all {allDay ? 'left-[22px]' : 'left-0.5'}"></span>
						</button>
						<span class="text-md text-text">All day</span>
						<div class="flex overflow-hidden rounded-md border border-border text-caption" role="radiogroup" aria-label="Show as">
							{#each [['busy', 'Busy'], ['oof', 'Out of office']] as const as [v, label] (v)}
								<button type="button" role="radio" aria-checked={showAs === v} class="px-2.5 py-1 {showAs === v ? 'bg-accent/25 text-text' : 'link-muted'}" onclick={() => (showAs = v)}>{label}</button>
							{/each}
						</div>
					{/if}
				</div>
