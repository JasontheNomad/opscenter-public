<script lang="ts">
	...
	import type { Task, TaskPatch, Company, Contact } from '$lib/types';
	...
	import { leaf, vaultHref } from '$lib/notes/vault';
		...
		<div class="flex flex-wrap items-end gap-x-6 gap-y-3">
			{#if task.company_id}
				<div>
					...
					{#if notesFolder}
						<a href="{vaultHref(notesFolder)}?from={encodeURIComponent(`${page.url.pathname}?task=${task.id}`)}&fromTitle={encodeURIComponent(task.title)}" class="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface-2 px-2.5 py-1 text-xs text-text hover:border-accent">
							📁 {leaf(notesFolder)} →
						</a>
					{:else}
