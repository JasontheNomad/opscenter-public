<script lang="ts">
	...
	import { MentionPicker } from '$lib/mentionPicker.svelte';
	import { matchPeople, type Person } from '$lib/mentions';
	...
	// @mentions: a chat offers its members, a channel searches the directory
	const directory = async (q: string): Promise<Person[]> =>
		q.trim().length < 2 ? [] : (await api<Person[]>(`/api/teams/people?q=${encodeURIComponent(q.trim())}`)).map(({ id, name }) => ({ id, name }));
	const picker = new MentionPicker((q) => ('chat' in target ? matchPeople(callPeople, q) : directory(q)), () => ('chat' in target ? 0 : 200));
	const threadPicker = new MentionPicker(directory, () => 200);
	...
	async function sendReply() {
		...
		if (!text) return;
		threadDraft = '';
		const people = threadPicker.take(text);
		...
		await post('/api/teams/replies', { team: target.team, channel: target.channel, message: m.id, text, mentions: people }).catch((e) => ((sendError = errMsg(e)), (threadDraft = text), threadPicker.keep(people)));
		await invalidateAll();
		extraReplies[m.id] = [];
	}
	...
	async function send() {
		...
		try {
			await post('/api/teams/send', { ...target, text, quote: q, uploads: ups.map(payload), mentions: people });
			await invalidateAll();
		} catch (e) {
			sendError = errMsg(e);
...
<div class="flex min-w-0 flex-1 flex-col">
	...
	<form class="relative flex shrink-0 flex-col gap-2 border-t p-3 {dragOver ? 'border-accent bg-accent/5' : 'border-border'}" onsubmit={(e) => (e.preventDefault(), send())}
		ondragover={(e) => { e.preventDefault(); dragOver = true; }} ondragleave={() => (dragOver = false)} ondrop={onDrop}>
		{@render mentionMenu(picker, box, (v) => (draft = v))}
		...
		<textarea
			...
			onkeydown={(e) => { if (box && picker.key(e, box, (v) => (draft = v))) return; if (e.key === 'Enter' && !e.shiftKey) (e.preventDefault(), void send()); if (e.key === 'Escape') quote = null; }}
			oninput={() => { if (box) void picker.update(box); }}
			onblur={() => picker.close()}
			onpaste={onPaste}
		></textarea>
...
{#if threadRoot}
	<aside class="flex w-[420px] shrink-0 flex-col border-l border-border bg-surface">
		...
		<form class="relative shrink-0 border-t border-border p-3" onsubmit={(e) => (e.preventDefault(), sendReply())}>
			{@render mentionMenu(threadPicker, threadBox, (v) => (threadDraft = v))}
			<div class="mb-1 text-caption text-muted">Send to: <span class="text-accent">thread only</span></div>
			<div class="flex gap-2">
				<textarea bind:this={threadBox} bind:value={threadDraft} rows="2" placeholder="Reply in thread" class="min-w-0 flex-1 resize-none rounded-md border border-border bg-surface-2 px-3 py-2 text-body focus-accent"
					onkeydown={(e) => { if (threadBox && threadPicker.key(e, threadBox, (v) => (threadDraft = v))) return; if (e.key === 'Enter' && !e.shiftKey) (e.preventDefault(), void sendReply()); if (e.key === 'Escape') threadId = null; }}
					oninput={() => { if (threadBox) void threadPicker.update(threadBox); }}
					onblur={() => threadPicker.close()}></textarea>
...
<!-- @mention list above a composer; mousedown keeps focus in the textarea so blur doesn't close it first -->
{#snippet mentionMenu(p: MentionPicker, el: HTMLTextAreaElement | undefined, set: (v: string) => void)}
	{#if el && p.items.length}
		<div class="absolute bottom-full left-3 z-30 mb-1 w-72 rounded-md border border-border bg-surface p-1 shadow-xl">
			{#each p.items as person, i (person.id)}
				<button type="button" class="menu-item {i === p.index ? 'bg-surface-2 text-text' : 'menu-idle'}" onmouseenter={() => (p.index = i)} onmousedown={(e) => { e.preventDefault(); p.pick(person, el, set); }}>
					<Avatar id={person.id} name={person.name} size={20} /><span class="truncate">{person.name}</span>
				</button>
			{/each}
		</div>
	{/if}
