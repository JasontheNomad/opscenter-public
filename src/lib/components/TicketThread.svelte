<script lang="ts">
	...
	// messages + emails), each with its composer, plus Notes — the client's own vault notes (its subfolders'
	// too), the same ones the Vault page shows, opened in their own window.
	import { api, post as apiPost, errMsg } from '$lib/api';
	import { leaf } from '$lib/notes/vault';
	...
	type VaultNote = { folder: string; file: string; title: string; excerpt: string; updated_at: string; pinned: boolean };
	...
	// New note from the ticket: the same vault file the Vault page writes, so it shows up there too.
	// A company with no folder yet gets one (and the link), the way a new ticket makes it.
	...
	async function pinNote(folder: string, file: string, pinned: boolean) {
		await apiPost('/api/notes/pin', { client: folder, file, pinned });
		await loadVault(task.id);
	}
	...
	{#if tab === 'vault'}
		...
		{:else}
			<div class="mb-1 flex items-center gap-2">
				<span class="field-label flex-1">{vault.client ? leaf(vault.client) : (task.company_name ?? 'This ticket')} · client notes</span>
				{#if task.company_id}
			...
			<div class="flex flex-col">
				{#each vault.notes as n (n.folder + '/' + n.file)}
					<div class="group relative rounded-md hover:bg-surface-2/60">
						<button
							class="block w-full px-2 py-2 pr-7 text-left"
							title="Open in its own window"
							onclick={() => noteWindows.open(n.folder, n.file)}
						>
							<div class="truncate text-body text-text">{n.title}</div>
							<div class="truncate text-caption text-muted">{n.excerpt || '—'}</div>
							<!-- a note from a subfolder says which one -->
							<div class="truncate text-2xs text-muted">{vault.client && n.folder !== vault.client ? `${n.folder.slice(vault.client.length + 1)} · ` : ''}{shortDate(n.updated_at)}</div>
						</button>
						<button
							...
							onclick={() => pinNote(n.folder, n.file, !n.pinned)}
