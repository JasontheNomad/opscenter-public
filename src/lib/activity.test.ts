...
import { clientThread, descriptionText, portalComment, teamNotes } from './activity';
...
describe('portalComment', () => {
	it('takes the author from the header line and drops it', () => {
		expect(portalComment('Portal reply from client@firm.example:\nhttps://x.example/1\n\nthanks')).toEqual({
			author: 'client@firm.example',
			body: 'https://x.example/1\n\nthanks'
		});
	});

	it('keeps text without a header as-is', () => {
		expect(portalComment(' any news? ')).toEqual({ author: 'Client (portal)', body: 'any news?' });
		expect(portalComment('line one\nsent from client@firm.example: later')).toEqual({ author: 'Client (portal)', body: 'line one\nsent from client@firm.example: later' });
	});
});
