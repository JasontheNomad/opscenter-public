...
import { nestClientFolders } from '$lib/server/notes';
// one-time vault move (client folders under the clients folder) — before the pollers, whose first sync may make client folders
try {
	nestClientFolders();
} catch (e) {
	console.error('[vault] moving client folders under the clients folder failed:', e);
}
