<script lang="ts">
	...
	// all-day events arrive as UTC midnight -> compare the date part, not the local instant. `end` is the
	// day after the last one, so a multi-day event shows on every day it covers.
	const allDay = (d: Date) => data.events.filter((e) => e.allDay && e.start.slice(0, 10) <= ymd(d) && ymd(d) < e.end.slice(0, 10));
						...
						<div class="relative border-l border-border" style="height: {HOURS.length * H}px" ondblclick={(e) => { const y = e.offsetY; const at = new Date(d); at.setHours(HOURS[0] + Math.floor(y / H), Math.floor((y % H) / (H / 2)) * 30, 0, 0); openNew(at); }}>
							...
							{#each forDay(d) as e (e.id)}
								<button
									...
									onclick={() => (selected = e)}
									ondblclick={(ev) => {
										// the day column's dblclick means "new meeting here" — a meeting's own means "edit it"
										ev.stopPropagation();
										selected = e;
										if (e.isOrganizer && !e.cancelled && !e.allDay) editing = { id: e.id, series: false };
									}}
								>
