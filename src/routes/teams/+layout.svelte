<script lang="ts">
	...
	// sections — favorites keep the order of data.favs (draggable); chats (meeting chats too) are newest first
	...
	// the server list is already newest first. A meeting chat is listed too — hidden, its unread kept a
	// bubble nobody could clear.
	const chats = $derived(data.chats.filter((c) => !favSet.has(c.id)));
	// "Show more": the 8 newest, then 10 more per press
	let shownChats = $state(8);
	// drag state (svelte-dnd-action needs mutable arrays)
	// writable derived: follows the data after every poll, and a drag reassigns it in between
	...
	// params arrive decoded; decoding again threw on any id holding a literal %
	const activeChat = $derived(page.params.chat ?? '');
	const activeChannel = $derived(page.params.channel ?? '');
		...
		<aside class="flex w-72 shrink-0 flex-col border-r border-border" style={textStyle(data.textSize, 'teamsList')}>
			...
			<div class="flex-1 overflow-y-auto px-2 pb-4">
				...
				{#if showChats}
					...
					{#if open.chats}
						{@const list = chats.filter(passChat)}
						{#each list.slice(0, shownChats) as c (c.id)}{@render chatRow(c)}{/each}
						{#if list.length > shownChats}
							<button class="flex h-8 w-full items-center gap-1.5 rounded-md px-2 text-caption link-muted hover:bg-surface-2/60" onclick={() => (shownChats += 10)}>Show more <span class="text-2xs">▼</span></button>
						{/if}
					{/if}
				{/if}
