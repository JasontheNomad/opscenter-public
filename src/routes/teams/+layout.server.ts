import { teamsConfigured, teamsConnected, cachedChats, listTeams, favorites, me, applyTeamOrder, teamsStatus, markChannelSeen, markChatSeenLocal, notifyPrefs, activityUnseen } from '$lib/server/teams';
...
const empty = (configured: boolean, connected: boolean, error: string | null) => ({
	...
	presence: {} as Record<string, string>, myPresence: 'Available',
...
export const load: LayoutServerLoad = async ({ params }) => {
	...
	try {
		const openChat = params.chat || null; // already decoded
		...
		return { configured, connected, chats, teams, favs, channelLast: st.channelLast, unreadChannelIds, unreadChannels, presence: st.presence, myPresence: st.myPresence ?? 'Available', notify: notifyPrefs(), mentions: activityUnseen('mention'), threads: activityUnseen('thread'), me: my, error: st.presenceError ?? null };
	} catch (e) {
