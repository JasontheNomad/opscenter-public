...
export const PATCH: RequestHandler = async ({ params, request }) => {
	const { subject, attendees, optional, location, body, start, end } = await readBody(request);
	...
	// absent (not empty) = an editor from before optional attendees; updateMeeting keeps existing types
	const optionalList = Array.isArray(optional) ? optional.filter(isEmail) : undefined;
	return upstream(() =>
		updateMeeting(params.id, {
			subject: subject.trim(), attendees: list, optional: optionalList, location: typeof location === 'string' ? location.trim() : '',
