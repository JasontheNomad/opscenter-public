...
import { sendThreadReply, pickMentions } from '$lib/server/teams';
import { pickUploads } from '$lib/server/uploads';
import { need, upstream, readBody } from '$lib/server/http';
import type { RequestHandler } from './$types';
export const POST: RequestHandler = async ({ request }) => {
	const { team, channel, message, text, uploads, mentions } = await readBody(request);
	...
	return upstream(() => sendThreadReply(t_, c_, m_, t, ups, pickMentions(mentions)), 201);
};
