...
import { sendChatMessage, sendChannelMessage, invalidateChat, pickMentions, type Quote } from '$lib/server/teams';
import { pickUploads } from '$lib/server/uploads';
import { upstream, readBody } from '$lib/server/http';
import type { RequestHandler } from './$types';
export const POST: RequestHandler = async ({ request }) => {
	const { chat, team, channel, text, quote, uploads, mentions } = await readBody(request);
	const ups = pickUploads(uploads);
	const people = pickMentions(mentions);
	...
	const q: Quote | null =
		qo && typeof qo.id === 'string'
			...
			: null;
	if (typeof team === 'string' && typeof channel === 'string') return upstream(() => sendChannelMessage(team, channel, t, q, ups, people), 201);
	if (typeof chat !== 'string' || !chat) error(400, 'chat or team+channel required');
	return upstream(async () => {
		const created = await sendChatMessage(chat, t, q, ups, people);
		invalidateChat(chat);
		return created;
	}, 201);
};
