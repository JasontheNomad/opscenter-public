...
import { savedThreadId } from '$lib/server/seed';
...
export const GET: RequestHandler = ({ params }) => {
	...
	return upstream(async () => {
		const threadId = await savedThreadId({ ...task, hs_id: hsId });
		const [items, contact] = await Promise.all([ticketActivity(hsId, threadId), ticketContact(hsId).catch(() => null)]);
